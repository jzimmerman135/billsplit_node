...
function getName(){
    ...
    if (input.value.toLowerCase() == "quit" || input.value.toLowerCase() == "done"){
        ...
        if (document.getElementById("finishButton").disabled == false) {
            ...
            if (items.length == 0){
                updateMessage("Enter first item");
            } else {
...
function enableBackButton(type) {
    let back = document.getElementById("backButton");
    back.style.visibility = "visible";
    if (type == "names") {
        back.onclick = function () {
            document.getElementById("finishButton").onclick = function () {finishNames()};
            document.getElementById("inBar").onkeydown = function () {enterName(event)};
            document.getElementById("submitInput").onclick = function () {getName()};
            updateMessage("Add a name");
            back.style.visibility = "hidden";
        }
    }
    if (type == "items") {
        back.onclick = function () {
            document.getElementById("inputReg").style.display = "flex";
            document.getElementById("finishButton").onclick = function () {finishItems()};
            document.getElementById("inBar").onkeydown = function () {enterItem(event)};
            document.getElementById("submitInput").onclick = function () {getInput()};
            updateMessage("Add an item");
            document.getElementsByClassName("inReg")[0].style.margin = "0";
            document.getElementsByClassName("inReg")[0].style.marginLeft = "auto";
            hideTotals();
            document.getElementById("minusTax").onmousedown = minusTax;
            document.getElementById("plusTax").onmousedown = addTax;
            document.getElementById("plusTax").style.visibility = "visible";
            document.getElementById("minusTax").style.visibility = "visible";
            document.getElementById("sButtonBox").style.display = "none";
            back.onclick = function () {enableBackButton("names")};
        }
    }
}

function setFinalNames(){
    var namesHrow = document.getElementsByClassName("namesHeader")[0];
    var finalNames = namesHrow.getElementsByClassName("names");
    var finalInits = namesHrow.getElementsByClassName("initials");
...
function setFinalReceipt(){
    let finalItems = document.getElementsByClassName("items");
    let finalPrices = document.getElementsByClassName("prices");
    let finalSharedBy = document.getElementsByClassName("sharedBy");
...
function getInput(){
    ...
    if (promptCount == 0){
        ...
        document.getElementById("backButton").onclick = "none";
        document.getElementById("backButton").style.opacity = 0.45;
    }
        ...
        document.getElementById("finishButton").disabled = false;
        enableBackButton("names");
        document.getElementById("backButton").style.opacity = 0.8;
    }
    else{
        promptCount = 0;
        getInput();
...
function invalidate(id,val){
    if (id[0] == "i" && id[1] == "t"){
        ...
        let i = id.substring('names'.length);
        i = parseInt(i);
        val = cleanName(val);
        if (names.includes(val) && names[i-1] != val){
            return true;
        }
    }
...
function cleanSharedBy(initList){
    ...
    else if (initList.includes("all-") || initList.includes("all -")) {
        let exclude = initList.slice(initList.indexOf("-") + 1);
        exclude = cleanSharedBy(exclude);
        for (let i = 0; i < initials.length; i++) {
            if (!exclude.includes(initials[i])){
                cleaned += initials[i];
            }
        }
    }
...
function displayTotals(history = ""){
...
function hideTotals(){
    ...
    document.getElementById("typeBar").style.display = "flex";
    document.getElementById("fButtonBox").style.display = "flex";
}
...
function updateTotals(history = "") {
    ...
    if (history != "history"){
        for (let i = 0; i < totals.length; i++) { 
            if (i == payer){
                finalTotals[payer] = -newTotals[i].toFixed(2);
            }
            else {
                finalTotals[i] = newTotals[i].toFixed(2);
            }
        }
    }    
}
...
function updateTotalBar() {
    ...
    netTotal = (parseFloat(sum(prices)) * tax).toFixed(2);
    x.innerHTML = netTotal;
}
...
function makeSharedByArray(i) {
    var sharedByArray = new Array();
    var initList = sharedBy[i];
    for (let j = 0; j < initList.length; j++) {
        let name = names[initials.indexOf(initList[j])];
        sharedByArray.push(name);
    }
    return sharedByArray;
}

function makeReceiptJSON(title, usernames) {
...
function addSharedUser() {
    let saveForm = document.getElementById("sharedUsers");
    let sharedUsers = document.getElementsByName("sharedUser")
    if (sharedUsers.length > 0){
        if (sharedUsers[sharedUsers.length - 1].value == ""){
            sharedUsers[sharedUsers.length - 1].placeholder = "Please enter a user to share with";
            return;
        }
    }
...
function closeSaveMenu() {
    document.getElementsByClassName("saveArea")[0].style.display = "none";
}
...
function submitSavedReceipt() {
    ...
    document.save.receiptJSON = JSON.stringify(makeReceiptJSON(title.value, usernames));
